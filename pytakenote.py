...
class Note:
    ...
    def save_to_db(self):
        ...
        if self.id_:
            if self.check_note(self.id_):  # note being edited
                ...
                sql = ("INSERT INTO Notes(id, title, body, datetime)"
                       " values(?, ?, ?, ?)")
                c.execute(sql, (self.id_, self.title, self.body,
                                self.date_time))
        else:
    ...
    def print_all_notes(db):
        ...
        print("-" * 72)
        for note in notes:
            print("%-3d %-48s %-16s" % (note[0], shorten_string(note[1]),
                                        note[3]))
...
def create_db(filepath):
    create_table_sql = ("CREATE TABLE Notes (id INTEGER PRIMARY KEY, "
                        "title TEXT, body TEXT, datetime DATETIME);")
...
def main(argv):
    ...
    group.add_argument("-a", "--add", action="store_true", help="Add a note")
